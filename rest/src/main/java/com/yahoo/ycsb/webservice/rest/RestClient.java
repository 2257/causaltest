...
public class RestClient extends DB {
  ...
  private volatile Criteria requestTimedout = new Criteria(false);
    public static final String urlDB_PROPERTY = "database";
  ...
  public void init() throws DBException {
   ...
   System.out.println("databasename="+props.getProperty(urlDB_PROPERTY));
    ...
    execTimeout = Integer.valueOf(props.getProperty(EXEC_TIMEOUT, "10"))*1000 ;//* 1000;
  ...
  @Override
    ...
    try {
      ...
      System.out.println("[httpGet] "+endpoint+"___"+result+"__"+responseCode);
      //    st = System.currentTimeMillis();
     String r = result.get("response").toString();
      //   en = System.currentTimeMillis()-st;
		//	System.out.println("read invoke httpGet time "+en);

      Pattern p = Pattern.compile("\\?type=read\\&");
      Matcher m = p.matcher(endpoint);
      String correct_r = "tweet1";

      if(m.find() && !(r.equals(correct_r)))
      {
        System.out.println("violation:"+r+",,"+r.equals(correct_r));
      }
    } catch (Exception e) {
  ...
  @Override
  public List<String> read2(String endpoint, List<String> result ) {
    //System.out.println("restclient .read2");
    int responseCode;
    try {
      long st ;
      long en ;
      ...
      responseCode = httpGet_return(urlPrefix + endpoint, result);
      System.out.println("[httpGet2] "+endpoint/*+"___"+result+"__"+responseCode*/);
      //    st = System.currentTimeMillis();
     /* String r = result.get("response").toSlltring();
      //   en = System.currentTimeMillis()-st;
        //  System.out.println("read invoke httpGet time "+en);
...
//System.out.println("read2 result  "+result);
    return result;
  }
  ...
  private int httpGet(String endpoint, Map<String, ByteIterator> result) throws IOException
  {
    ...
    //System.out.println("httpget sleep time = "+(en-st)+" ms");
    ...
    //System.out.println("restclient_httpget:"+endpoint);
...
// Must avoid memory leak.
      ...
      //System.out.println("restClient_httpGet_result:"+result);
    ...
    //System.out.println("httpget execute time = "+(optime-st1) + "ms, "+(optime-st2) + " ms, "+(optime-st3) + " ms, "+(optime-st4) + " ms");
    return responseCode;
  }
  private int httpGet_return(String endpoint, List<String> result) throws IOException
  {
    long st1,st2,st3=0,st4;
    long en;
    long optime;
...
// Must avoid memory leak.
      ...
      timer.interrupt();
//System.out.println("httpreturn  "+responseContent.toString());
      if(!responseContent.toString().equals("-"))
      {
  ...
  class Timer implements Runnable {
    ...
    public void run() {
      try
      {
        //st = System.currentTimeMillis(); //System.out.println("Timer.timeout "+timeout);
        Thread.sleep(timeout);
        //en = System.currentTimeMillis();
        //  System.out.println("timer. "+ (en-st));
