...
public class PostgresxlAmazonWFR extends AmazonWFR
{
    int typid = 66;
    int loopStartIndex = 2;
    public List<String> request = new ArrayList<>();

    public PostgresxlAmazonWFR()
    {
    ...
    {
        return "?type=insert2&tableName=amazoncommoditytable&id="+commodity+"&col=descri&val="+descri2+"&time="+
                (new SimpleDateFormat("yyyy/MM/dd-HH:mm:ss:SSS").format(System.currentTimeMillis()));
    }
